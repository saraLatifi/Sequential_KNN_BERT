# -*- coding: UTF-8 -*-
import os
from util_timestamp_sample import *
import argparse
import csv
...
def main():
    ...
    parser.add_argument('-n', '--sample', dest='sample')
    args = parser.parse_args()

    dataset_name = args.dataset_name
    version = args.signature
    sample_perc = args.sample
    if not sample_perc:
        sample_perc = 5
    sample_perc = int(sample_perc)
    ...
    dataset = data_partition(output_dir + dataset_name + '.txt', original_setting=True, sample_percentage=sample_perc)
    ...
    for u in user_train:
        ...
        if len(user_train[u]) <= length:
            ...
            sequence_user_train_tr[u][sequence_id] = [(j[0],j[1]) for e,j in enumerate(user_train[u][-length:])]
            sequence_id += 1
        else:
            for i in range(0, len(user_train[u]) - (length-step), step):
                ...
                if len(curr_session) == length: 
                    ...
                    sequence_user_train_tr[u][sequence_id] = [(j[0],j[1]) for e,j in enumerate(curr_session)]
                else:
    ...
    for u in user_train:
        ...
        if len(user_train[u]) <= length:
            ...
            sequence_user_train[u][sequence_id] = [(j[0],j[1]) for e,j in enumerate(user_train[u][-length:])]
            sequence_id += 1
        else:
            for i in range(0, len(user_train[u]) - (length-step), step):
                curr_session = user_train[u][i:i+length]
                if len(curr_session) == length: 
                    ...
                    sequence_user_train[u][sequence_id] = [(j[0],j[1]) for e,j in enumerate(curr_session)]
                else:
        ...
        if len(user_train[u]) <= (test_length):
            ...
            last_user_train[u] = [(j[0],j[1]) for e,j in enumerate(user_train[u][-test_length:])]
        else:
    ...
    order_nums = {}
    output_filename = output_dir + 'sequence_aware_datasets/prepared/timestamps_sampled/split_seq/' + dataset_name + "_" + version
    ...
    for u in user_train:
        if u in user_test:
            ...
            last_user_train[u].extend([(user_test[u][0][0], user_test[u][0][1])])  # user_train contains all interactions of each user, except the last one
