...
flags.DEFINE_integer(
    "time_split", None,
    "Timestamp to split data.")

flags.DEFINE_float("prop_sliding_window", 0.1, "sliding window step size.")

flags.DEFINE_string(
    "data_dir", './data/sequence_aware_datasets/prepared_bert4rec/',
    "data dir.")
...
flags.DEFINE_bool("original_setting", True, "run in original setting of the BERT4Rec paper")
...
def main():
    ...
    if FLAGS.original_setting:
        output_dir = output_dir + 'original_setting/'

    else:
        dataset = data_partition(output_dir + dataset_name + '.txt', FLAGS.original_setting)
    ...
    if FLAGS.original_setting:
        ...
        vocab = FreqVocab(user_train_data)  # If we create vocab with test then there are missing items b/c sequences are skipped
